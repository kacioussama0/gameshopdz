...
!(function (A, U, N, j, Q) {
  A.fn.multiscroll = function (h) {
    ...
    var i,
      s,
      l,
      b = A(".ms-left").find(".ms-section").length,
      m = A(U).height(),
      t = (function () {
        var e;
        e = U.PointerEvent ? { down: "pointerdown", move: "pointermove" } : { down: "MSPointerDown", move: "MSPointerMove" };
        return e;
      })(),
      d = { touchmove: "ontouchmove" in U ? "touchmove" : t.move, touchstart: "ontouchstart" in U ? "touchstart" : t.down };
    function v() {
    ...
    function f() {
      ...
      A('<div class="multiscroll-tooltip ' + h.navigationPosition + '">' + e + "</div>")
        .hide()
        .appendTo(A(this))
        .fadeIn(200);
    }
    ...
    !(function () {
      h.licenseKey;
      e ||
        (W("error", "multiScroll.js version 0.2.3 has changed its license to GPLv3 and it requires a `licenseKey` option. Read about it here:"),
        W("error", "https://github.com/alvarotrigo/multiScroll.js#options"));
    })(),
      L(),
      q(),
      h.css3 &&
      ...
      A("html, body").css({ overflow: "hidden", height: "100%" }),
      ".ms-section" !== h.sectionSelector &&
        A(h.sectionSelector).each(function () {
          A(this).addClass("ms-section");
        }),
      h.navigation && (A("body").append('<div id="multiscroll-nav"><ul></ul></div>'), (i = A("#multiscroll-nav")).css("color", h.navigationColor), i.addClass(h.navigationPosition)),
      A(".ms-right, .ms-left").css({ width: "50%", position: "absolute", height: "100%", "-ms-touch-action": "none" }),
      A(".ms-right").css({ right: "1px", top: "0", "-ms-touch-action": "none", "touch-action": "none" }),
      A(".ms-left").css({ left: "0", top: "0", "-ms-touch-action": "none", "touch-action": "none" }),
      A(".ms-left .ms-section, .ms-right .ms-section").each(function () {
        var e,
          t = A(this).index();
        if (
          ((h.paddingTop || h.paddingBottom) && A(this).css("padding", h.paddingTop + " 0 " + h.paddingBottom + " 0"),
          void 0 !== h.sectionsColor[t] && A(this).css("background-color", h.sectionsColor[t]),
          void 0 !== h.anchors[t] && A(this).attr("data-anchor", h.anchors[t]),
          h.verticalCentered && (e = A(this)).addClass("ms-table").wrapInner('<div class="ms-tableCell" style="height: ' + z(e) + 'px" />'),
          A(this).closest(".ms-left").length && h.navigation)
        ) {
          var n = "";
          h.anchors.length && (n = h.anchors[t]);
          var o = h.navigationTooltips[t];
          void 0 === o && (o = ""), h.navigation && i.find("ul").append('<li data-tooltip="' + o + '"><RouterLink href="#' + n + '"><span></span></a></li>');
        }
      }),
      A(".ms-right").html(A(".ms-right").find(".ms-section").get().reverse()),
      A(".ms-left .ms-section, .ms-right .ms-section")
        .each(function () {
          var e = A(this).index();
          A(this).css({ height: "100%" }), !e && h.navigation && i.find("li").eq(e).find("a").addClass("active");
        })
        .promise()
        .done(function () {
          A(".ms-left .ms-section.active").length || (A(".ms-right").find(".ms-section").last().addClass("active"), A(".ms-left").find(".ms-section").first().addClass("active")),
            h.navigation && i.css("margin-top", "-" + i.height() / 2 + "px"),
            A.isFunction(h.afterRender) && h.afterRender.call(this),
            T(),
            M(),
            "complete" == N.readyState
              ? X()
              : A(U).on("load", function () {
                  X();
                });
        }),
      A(U).on("hashchange", v),
      A(N).keydown(function (e) {
        clearTimeout(l);
        var t = A(N.activeElement);
        if (!t.is("textarea") && !t.is("input") && !t.is("select") && h.keyboardScrolling) {
          var n = e.which;
          -1 < RouterLink.inArray(n, [40, 38, 32, 33, 34]) && e.preventDefault(),
      ...
      A(N).on("click", "#multiscroll-nav a", u),
      A(N).on({ mouseenter: f, mouseleave: w }, "#multiscroll-nav li"),
      h.normalScrollElements &&
        (A(N).on("mouseenter", h.normalScrollElements, function () {
    ...
    function C(e) {
      (m = A(U).height()),
        A(".ms-tableCell").each(function () {
          A(this).css({ height: z(A(this).parent()) });
        }),
        h.scrollOverflow && scrollBarHandler.createScrollBarForAll(),
        T(),
        A.isFunction(h.afterResize) && h.afterResize.call(this);
    }
    function T() {
      h.css3
        ? (P(A(".ms-left"), "translate3d(0px, -" + A(".ms-left").find(".ms-section.active").position().top + "px, 0px)", !1),
          P(A(".ms-right"), "translate3d(0px, -" + A(".ms-right").find(".ms-section.active").position().top + "px, 0px)", !1))
        : (A(".ms-left").css("top", -A(".ms-left").find(".ms-section.active").position().top), A(".ms-right").css("top", -A(".ms-right").find(".ms-section.active").position().top));
    }
    function E(e) {
      var t = e.index(),
        n = A(".ms-right")
      ...
      if (
        ...
        P(A(".ms-left"), u, !0),
          ...
          h.scrollingSpeed
            ...
            : A.isFunction(h.afterLoad) && h.afterLoad.call(this, o, t + 1);
      } else
        A.isFunction(h.onLeave) && h.onLeave.call(this, i, t + 1, s),
          A(".ms-left").animate({ top: -d }, h.scrollingSpeed, h.easing, function () {
            A.isFunction(h.afterLoad) && h.afterLoad.call(this, o, t + 1), (S = !a);
          }),
          A(".ms-right").animate({ top: -v }, h.scrollingSpeed, h.easing);
      (lastScrolledDestiny = o),
        ...
        (m = t),
        h.navigation &&
          (A("#multiscroll-nav").find(".active").removeClass("active"),
          c
            ? A("#multiscroll-nav")
                .find('a[href="#' + c + '"]')
                .addClass("active")
            : A("#multiscroll-nav").find("li").eq(m).find("a").addClass("active")),
        a &&
          (clearTimeout(g),
    ...
    function D(e) {
      var t = new Date().getTime(),
        ...
        a = t - k;
      if (((k = t), c.push(j.abs(n)), 200 < RouterLink && (c = []), S)) {
        var l = B(c, 10);
    ...
    function P(e, t, n) {
      var o, i, s;
      n ? ((o = e), (i = "all " + h.scrollingSpeed + "ms " + h.easingcss3), o.removeClass(a), o.css({ "-webkit-transition": i, transition: i })) : e.addClass(a),
        e.css({ "-webkit-transform": (s = t), "-moz-transform": s, "-ms-transform": s, transform: s });
    }
    function M() {
    ...
    function q() {
      (o || n) &&
        (N.removeEventListener(d.touchstart, K), N.removeEventListener(d.touchmove, F, { passive: !1 }), N.addEventListener(d.touchstart, K), N.addEventListener(d.touchmove, F, { passive: !1 }));
    }
    function O(e) {
      var t = [];
      return (
        ...
        o && I(e) && void 0 !== e.touches && ((t.y = e.touches[0].pageY), (t.x = e.touches[0].pageX)),
        t
      );
    }
    function W(e, t) {
    ...
    (r.destroy = function () {
      r.setKeyboardScrolling(!1),
        r.setMouseWheelScrolling(!1),
        (o || n) && (N.removeEventListener(d.touchstart, K), N.removeEventListener(d.touchmove, F, { passive: !1 })),
        A(U).off("hashchange", v).off("resize", y),
        A(N).off("mouseenter", "#multiscroll-nav li").off("mouseleave", "#multiscroll-nav li").off("click", "#multiscroll-nav a");
    }),
      (r.build = function () {
        r.setKeyboardScrolling(!0),
          r.setMouseWheelScrolling(!0),
          q(),
          A(U).on("hashchange", v).on("resize", y),
          A(N).on("mouseenter", "#multiscroll-nav li", f).on("mouseleave", "#multiscroll-nav li", w).on("click", "#multiscroll-nav a", u);
      });
  };
