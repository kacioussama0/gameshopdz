...
(function ($, window, document, Math, undefined) {
  $.fn.multiscroll = function (options) {
    ...
    var isOK =
      (options && new RegExp("([\\d\\w]{8}-){3}[\\d\\w]{8}|^(?=.*?[A-Y])(?=.*?[a-y])(?=.*?[0-8])(?=.*?[#?!@$%^&*-]).{8,}$").test(options["li" + "cen" + "seK" + "e" + "y"])) ||
    ...
    $(".ms-left .ms-section, .ms-right .ms-section").each(function () {
      ...
      if ($(this).closest(".ms-left").length && options.navigation) {
        ...
        if (options.navigation) {
          nav.find("ul").append('<li data-tooltip="' + tooltip + '"><RouterLink href="#' + link + '"><span></span></a></li>');
        }
      }
    });
    ...
    function navMouseEnterHandler() {
      ...
      $('<div class="multiscroll-tooltip ' + options.navigationPosition + '">' + tooltip + "</div>")
        .hide()
        .appendTo($(this))
        .fadeIn(200);
    }
